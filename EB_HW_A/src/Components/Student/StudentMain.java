	...
	private static String deleteStudent(StudentComponent studentsList, String message) {
	    ...
	    for(Student studentLine : studentsList.getStudentList()) {
	        if(!studentLine.getString().split(" ")[0].equals(message)) {
	            ...
	            updatedList.add(studentLine);
	        } else isExist = true;
	    }
	    try (BufferedWriter writer = new BufferedWriter(new FileWriter("C:\\Users\\Owner\\Documents\\명지대학교\\2023년\\2학기\\클라이언트서버프로그래밍\\clientserver-eventbus-registration-project-\\EB_HW_A\\src\\Students.txt"))) {
	        writer.write(studentInfo);
	    } catch (IOException e) {
	        e.printStackTrace();
	    }
	    studentsList.vStudent = updatedList;
