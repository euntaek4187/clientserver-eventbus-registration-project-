...
public class CourseMain {
	...
	private static String deleteCourse(CourseComponent coursesList, String message) {
	    ...
	    ArrayList<Course> updatedList = new ArrayList<Course>();
	    for(Course courseLine : coursesList.getCourseList()) {
	        if(!courseLine.getString().split(" ")[0].equals(message)) {
	            ...
	            updatedList.add(courseLine);
	        } else isExist = true;
	    }
	    try (BufferedWriter writer = new BufferedWriter(new FileWriter("C:\\Users\\Owner\\Documents\\명지대학교\\2023년\\2학기\\클라이언트서버프로그래밍\\clientserver-eventbus-registration-project-\\EB_HW_A\\src\\Courses.txt"))) {
	        writer.write(courseInfo);
	    } catch (IOException e) {
	        e.printStackTrace();
	    }
	    coursesList.vCourse = updatedList;
