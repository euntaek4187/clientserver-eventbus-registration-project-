...
public class CourseMain {
	public static String fileName = "C:\\Users\\Owner\\Documents\\명지대학교\\2023년\\2학기\\클라이언트서버프로그래밍\\clientserver-eventbus-registration-project-\\EB_HW_B2\\src\\Courses.txt";
		...
		while (!done) {
			...
			for (int i = 0; i < eventQueue.getSize(); i++) {
				...
				case CheckPrerequisite:
					printLogEvent("Get", event);
				    validationBeforePrerequisiteCheck(event.getMessage(), coursesList, eventBus);
					break;
				case QuitTheSystem:
	...
	private static void validationBeforePrerequisiteCheck(String message, CourseComponent coursesList, RMIEventBus eventBus) throws InterruptedException, RemoteException {
	...
	private static void updateCourseDataToFile(CourseComponent coursesList) {
	...
	private static String addCourse(CourseComponent coursesList, String message) {
		...
		if (!coursesList.isRegisteredCourse(course.courseId)) {
			...
			updateCourseDataToFile(coursesList);
